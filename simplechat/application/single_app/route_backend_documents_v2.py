...
def register_route_backend_documents(app):
    ...
    def get_file_content():
        ...
        print(f"DEBUG get_file_content: user_id={user_id}, conversation_id={conversation_id}, file_id={file_id}")
    ...
    def api_user_upload_document():
        ...
        for file in files:
            ...
            print(f"DEBUG: Generated document_id: {parent_document_id} for file: {original_filename}")
            
            temp_file_path = None
            try:
                sc_temp_files_dir = "/sc-temp-files" if os.path.exists("/sc-temp-files") else ""
                print(f"DEBUG: Using temp directory: {sc_temp_files_dir if sc_temp_files_dir else 'system temp'}")
            ...
            try:
                ...
                print(f"DEBUG: Creating document in Cosmos DB with document_id={parent_document_id}, user_id={user_id}")
                ...
                print(f"DEBUG: Document Intelligence endpoint: {os.getenv('AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT', 'NOT SET')}")
                print(f"DEBUG: Document Intelligence key exists: {'Yes' if os.getenv('AZURE_DOCUMENT_INTELLIGENCE_KEY') else 'No'}")
                ...
                print(f"DEBUG: Successfully queued document for processing: {parent_document_id}")
        ...
        if not processed_docs and upload_errors: response_status = 400
    ...
    def api_get_user_documents():
        ...
        try:
            ...
            for doc in docs:
                print(f"  - Document: id={doc.get('id')}, file_name={doc.get('file_name')}, status={doc.get('status')}")
    ...
    def api_delete_user_document(document_id):
        ...
        try:
            ...
            try:
                ...
                print(f"DEBUG: Found document to delete: {existing_doc.get('file_name')}")
            except Exception as e:
