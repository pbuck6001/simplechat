...
def register_route_frontend_chats(app):
    ...
    def chats():
        ...
        enable_enhanced_citations = public_settings.get(
            "enable_enhanced_citations", False
        )
        enable_document_classification = public_settings.get(
            "enable_document_classification", False
        )
        enable_extract_meta_data = public_settings.get(
    ...
    def upload_file():
        ...
        try:
            file_message_id = (
                f"{conversation_id}_file_{int(time.time())}_{random.randint(1000,9999)}"
            )
            file_message = {
                "id": file_message_id,
            ...
            return (
                jsonify({"error": f"Error adding file to conversation: {str(e)}"}),
                500,
            )
    ...
    def view_document():
        ...
        if not user_id:
            return (
                ...
                401,
            )  # Should be caught by @login_required anyway
        ...
        raw_doc = (
            doc_response.get_json()
        )  # Assuming get_user_document returns jsonify response
        ...
        try:
            ...
            container_name = (
                storage_account_user_documents_container_name  # From config
            )
            ...
            if not all([blob_service_client, storage_account_key, container_name]):
                return (
                    jsonify(
                        {
                            "error": "Internal server error: Storage access not configured."
                        }
                    ),
                    500,
                )
            ...
            sas_token = generate_blob_sas(
                ...
                + timedelta(minutes=10),  # Short expiry for view access
            )
        ...
        try:
                ...
                try:
                    ...
                    if current_idx < 0 or current_idx >= total_pages:
                        return (
                            jsonify(
                                {
                                    "error": f"Requested page ({page_number}) out of range (Total: {total_pages})"
                                }
                            ),
                            400,
                        )
                    ...
                    extracted_pdf.save(
                        local_file_path, garbage=3, deflate=True
                    )  # garbage=3 is often sufficient
                    ...
                    response = send_file(
                        local_file_path, as_attachment=False, mimetype="application/pdf"
                    )
                    response.headers["X-Sub-PDF-Page"] = str(new_page_number)
            ...
            # --- Image Handling (Send file directly) ---
                ...
                # File will be cleaned up in 'finally' block after response is sent
                return send_file(
                    local_file_path, as_attachment=False, mimetype=mimetype
                )
            ...
            # --- Fallback for unsupported types, PPTX, DOCX, etc. ---
                ...
                return (
                    jsonify(
                        {"error": f"Unsupported file type for viewing: {file_ext}"}
                    ),
                    415,
                )
            else:
            ...
            return (
                jsonify({"error": "Internal processing error: File access issue"}),
                500,
            )
        except Exception as e:
            ...
            return (
                jsonify(
                    {"error": f"An internal error occurred processing the document."}
                ),
                500,
            )
        finally:
    ...
    @user_required
    @enabled_required("enable_user_workspace")
    def view_document_enhanced(doc_id):
        """
        ...
        try:
            ...
            sas_token = generate_blob_sas(
                ...
                permission=BlobSasPermissions(read=True),
                expiry=datetime.utcnow()
                + timedelta(hours=1),  # Longer expiry for full document
            )
            ...
            if range_header:
                ...
                flask_response = Response(
                    generate(), status=response.status_code, mimetype="application/pdf"
                )
                ...
                flask_response = Response(generate(), mimetype="application/pdf")
                flask_response.headers["Accept-Ranges"] = "bytes"
                flask_response.headers["Content-Length"] = response.headers.get(
                    "Content-Length", ""
                )
