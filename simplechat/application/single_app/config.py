...
CUSTOM_COGNITIVE_SERVICES_URL_VALUE = os.getenv(
    "CUSTOM_COGNITIVE_SERVICES_URL_VALUE", ""
)
...
SCOPE = [
    ...
    "People.Read.All",
    "Group.Read.All",
]  # Adjust scope according to your needs
MICROSOFT_PROVIDER_AUTHENTICATION_SECRET = os.getenv(
    "MICROSOFT_PROVIDER_AUTHENTICATION_SECRET"
)

OIDC_METADATA_URL = f"https://login.microsoftonline.com/{TENANT_ID}/v2.0/.well-known/openid-configuration"
AZURE_ENVIRONMENT = os.getenv(
    "AZURE_ENVIRONMENT", "public"
)  # public, usgovernment, custom
...
print(f"Using Azure Cosmos DB endpoint: {cosmos_endpoint}")
cosmos_key = os.getenv("AZURE_COSMOS_KEY")
print(
    f"Using Azure Cosmos DB key: {'*' * 8}{cosmos_key[-8:]}"
    if cosmos_key
    else "No Cosmos DB key provided"
)

cosmos_authentication_type = os.getenv(
    "AZURE_COSMOS_AUTHENTICATION_TYPE", "key"
)  # key or managed_identity
...
cosmos_conversations_container = cosmos_database.create_container_if_not_exists(
    id=cosmos_conversations_container_name, partition_key=PartitionKey(path="/id")
)
...
cosmos_group_documents_container = cosmos_database.create_container_if_not_exists(
    id=cosmos_group_documents_container_name, partition_key=PartitionKey(path="/id")
)
...
cosmos_user_prompts_container = cosmos_database.create_container_if_not_exists(
    id=cosmos_user_prompts_container_name, partition_key=PartitionKey(path="/id")
)
...
cosmos_group_prompts_container = cosmos_database.create_container_if_not_exists(
    id=cosmos_group_prompts_container_name, partition_key=PartitionKey(path="/id")
)
    ...
    if not custom_logo_b64:
        ...
        if os.path.exists(logo_path):
            try:
                os.remove(logo_path)
                print(
                    f"Removed existing {logo_filename} as custom logo is disabled/empty."
                )
...
def ensure_custom_favicon_file_exists(app, settings):
    ...
    try:
        ...
        print(
            f"Unexpected error during favicon file write for {favicon_filename}: {ex}"
        )
...
def initialize_clients(settings):
    ...
    with CLIENTS_LOCK:
        ...
        enable_document_intelligence_apim = settings.get(
            "enable_document_intelligence_apim"
        )
        azure_apim_document_intelligence_endpoint = settings.get(
            "azure_apim_document_intelligence_endpoint"
        )
        azure_apim_document_intelligence_subscription_key = settings.get(
        ...
        azure_apim_ai_search_subscription_key = settings.get(
        ...
        try:
            if enable_document_intelligence_apim:
                ...
                if (
                    settings.get("azure_document_intelligence_authentication_type")
                    == "managed_identity"
                ):
                    document_intelligence_client = DocumentIntelligenceClient(
        ...
        try:
            if enable_ai_search_apim:
                ...
                if (
                    settings.get("azure_ai_search_authentication_type")
                    == "managed_identity"
                ):
                    search_client_user = SearchClient(
        ...
        if settings.get("enable_content_safety"):
            ...
            azure_apim_content_safety_endpoint = settings.get(
                "azure_apim_content_safety_endpoint"
            )
            azure_apim_content_safety_subscription_key = settings.get(
            ...
            if safety_endpoint and safety_key:
                try:
                    if enable_content_safety_apim:
                        ...
                        if (
                            settings.get("content_safety_authentication_type")
                            == "managed_identity"
                        ):
                            content_safety_client = ContentSafetyClient(
        ...
        try:
            if enable_enhanced_citations:
                blob_service_client = BlobServiceClient.from_connection_string(
                    settings.get("office_docs_storage_account_url")
                )
                ...
                for container_name in [
                    ...
                    try:
                        ...
                        print(
                            f"Error creating container {container_name}: {str(container_error)}"
                        )
